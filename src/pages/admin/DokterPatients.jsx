      ...
      <div className="rounded-2xl bg-gradient-to-r from-blue-600 to-blue-700 p-6 text-white shadow-lg shadow-blue-500/20 border border-white/10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-blue-100">Daftar pasien yang menunggu dilayani</p>
          </div>
          <div className="hidden sm:block">
            <UsersIcon className="h-16 w-16 text-blue-300/30" />
          </div>
        </div>
      </div>
        ...
        {patients.length === 0 ? (
          ...
          <div className="divide-y divide-white/5">
            {patients.map((patient) => (
              <Link key={patient.id} to={`/admin/patient/${patient.id}`} className="block p-6 hover:bg-white/5 transition-colors group">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="hidden sm:flex items-center gap-3 rounded-xl bg-blue-500/10 px-4 py-3 border border-blue-500/20">
                      <CalendarIcon className="h-5 w-5 text-blue-400" />
                      <div>
                        <p className="text-sm font-semibold text-blue-200">{formatDate(patient.slots?.date)}</p>
                        <p className="text-sm text-blue-400">{formatHour(patient.slots?.hour)}</p>
                      </div>
                    </div>
                ...
                {/* Mobile Schedule */}
                <div className="mt-4 sm:hidden flex items-center gap-3 rounded-xl bg-blue-500/10 px-4 py-3 border border-blue-500/20">
                  <CalendarIcon className="h-5 w-5 text-blue-400" />
                  <div>
                    <p className="text-sm font-semibold text-blue-200">{formatDate(patient.slots?.date)}</p>
                    <p className="text-sm text-blue-400">{formatHour(patient.slots?.hour)}</p>
